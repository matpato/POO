package aula07.calculator.console;
 ...
 * Each view has a coordinate (its upper left corner), a dimension (number of rows and columns)
 * and its background color.<br/>
...
public class View {
     ...
     * @param width of area
     */
    public void center(int height, int width) { setOrig( (height-this.height)/2, (width-this.width)/2 ); }
    ...
    public boolean isOver(View other) {
        return (top >= other.top && top <= other.top + other.height ||
                other.top >= top && other.top <= top + height)
          && (left >= other.left && left <= other.left + other.width ||
